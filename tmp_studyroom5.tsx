...
import { CardStateManager, useCardState, useCardStates } from "../lib/CardStateManager";
...
export default function VibeStudyRoom() {
  ...
  const deckCardStates = useCardStates(user?.id, studyQueue);
  ...
  const startReviewAll = () => {
    if (!deck) return;
    setStudyQueue(deck.cards ? deck.cards.map((c: any) => c.id) : []);
    setStudyMode("all");
    setCurrentIndex(0);
    setSessionCorrectCount(0);
    setSessionMasteryGained(0);
  ...
  const executeActiveCardDeletion = useCallback(async () => {
    ...
    try {
      ...
      setStudyQueue((prev) => prev.filter((id) => id !== currentCard.id));
      setDeleteCountdown(null);
      setIsEditing(false);
  ...
  const handleMark = useCallback(
    (remembered: boolean) => {
      ...
      if (currentCard) {
        ...
        const diff = newMasteryVal - oldMastery;
...
2. Ví dụ hoặc mẹo nhớ ngắn gọn (CHỈ ghi nếu có giá trị học tập, tuyệt đối cấm để mục trống).`;
  ...
  const handleAddCard = async () => {
    ...
    try {
      ...
      setStudyQueue((prev) => [...prev, newCardObj.id]);
  ...
  const handleRemoveCard = async (targetCard: Flashcard) => {
    ...
    try {
      ...
      setStudyQueue((prev) => prev.filter((id) => id !== targetCard.id));
  ...
  const handleVirtualPlayCard = useCallback((c: Flashcard) => {
    setStudyQueue((prevQueue) => {
      const indexInQueue = prevQueue.findIndex((id) => id === c.id);
      if (indexInQueue !== -1) {
  ...
  const handleSelectCardFromList = useCallback(
    (card: Flashcard) => {
      ...
      if (indexInQueue === -1) {
        ...
        setStudyQueue(targetQueue.map((c: any) => c.id));
        setStudyMode("all");
        setFinished(false);
        if (!isPinned) setDeepExplanation(null);
        else setIsMinimized(true);
      ...
      const finalIndex = targetQueue.findIndex((c: any) => c.id === card.id);
      if (finalIndex !== -1) {
  ...
  if (finished) {
    ...
    const sessionCardsMasteryAvg = studyQueue.length
      ? Math.round(
          studyQueue.reduce((sum, id) => sum + (Number(deckCardStates[id]?.mastery) || 0), 0) /
            studyQueue.length,
        )
      : 0;
